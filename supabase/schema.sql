...
--   custom_name  text  … ユーザーが付けたお気に入り名（null なら曲名を表示）
--   created_at   timestamptz
--
-- canvas_index / custom_name 列は後から追加したもの。既存環境では以下を
-- Supabase の SQL Editor で一度だけ実行して追加する（既存行も安全）。
--
...
--
-- お気に入り名の編集には update ポリシーも必要（未作成なら以下を実行）:
--
